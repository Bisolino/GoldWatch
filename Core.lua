...
SlashCmdList["GOLDWATCH"] = function(msg)
    GW.SafeCall(function()
        ...
        local validCommands = {
            ...
            ["goal"] = GW.UI.ToggleGoalWindow  -- Novo comando para meta
        }
...
function GW.Initialize()
    ...
    GW.DB = GoldWatchDB or {
        ...
        goal = {                -- Novo: sistema de meta
            target = 0,          -- Meta em cobre
            progress = 0,        -- Progresso atual em cobre
            startMoney = 0,      -- Dinheiro inicial quando a meta foi definida
            isActive = false
        }
    }
    GoldWatchDB = GW.DB
    ...
    GW.DB.goal = GW.DB.goal or {                           -- Garantir existência da meta
        target = 0,
        progress = 0,
        startMoney = 0,
        isActive = false
    ...
    -- Popup para resetar meta
    StaticPopupDialogs["GOLDWATCH_CONFIRM_RESET_GOAL"] = {
        text = "Tem certeza que deseja resetar toda a progressão da meta?",
        button1 = GW.L.GetString("YES"),
        button2 = GW.L.GetString("NO"),
        OnAccept = function()
            GW.Data.ResetGoal()
    ...
    print(format(GW.L.GetString("LOADED"), "1.2"))
end
