...
function GW.UI.Create()
    ...
    -- Aumentar o tamanho do frame para acomodar a meta
    ...
    -- Mensagem de compatibilidade multi-versão
    ...
    -- Botão de fechar (X)
    GW.UI.closeBtn = CreateFrame("Button", nil, GW.UI.frame, "UIPanelCloseButton")
    GW.UI.closeBtn:SetSize(27, 27)
    GW.UI.closeBtn:SetPoint("TOPRIGHT", GW.UI.frame, "TOPRIGHT", -3, -3)
    GW.UI.closeBtn:SetScript("OnClick", function()
        GW.UI.frame:Hide()
    end)
    
    -- Botão de configurações (reposicionado)
    GW.UI.gearBtn = CreateFrame("Button", nil, frame)
    GW.UI.gearBtn:SetSize(22, 22)
    GW.UI.gearBtn:SetPoint("TOPRIGHT", GW.UI.closeBtn, "TOPLEFT", -5, -2)
    ...
    headerBg:SetPoint("TOPLEFT", 15, -55)
    headerBg:SetPoint("TOPRIGHT", -15, -55)
    ...
    -- Seção de Meta de Gold
    local goalBg = frame:CreateTexture(nil, "ARTWORK")
    goalBg:SetTexture("Interface\\Tooltips\\UI-Tooltip-Background")
    goalBg:SetVertexColor(0.1, 0.1, 0.1, 0.5)
    goalBg:SetPoint("TOP", lastSessionBg, "BOTTOM", 0, -30)
    goalBg:SetPoint("LEFT", 15, 0)
    goalBg:SetPoint("RIGHT", -15, 0)
    goalBg:SetHeight(40)
    
    GW.UI.labels.goal = CreateLabel(frame, GW.L.GetString("GOAL_LABEL"), "TOPLEFT", goalBg, "TOPLEFT", 10, -5, HEADER_COLOR, "GameFontNormal")
    GW.UI.goalText = CreateLabel(frame, "0/0", "TOPRIGHT", goalBg, "TOPRIGHT", -10, -5, VALUE_COLOR, "GameFontHighlight")
    ...
    buttonContainer:SetPoint("BOTTOM", -37, 10)
    buttonContainer:SetSize(360, 30)
    ...
    -- Botão de Meta
    GW.UI.goalBtn = CreateButton("GW_GoalBtn", GW.L.GetString("GOAL_BUTTON"), function() 
        GW.UI.ToggleGoalWindow()
    end, btnWidth, 25, "LEFT", GW.UI.historyBtn, "RIGHT", btnSpacing, 0)
...
function GW.UI.CreateMinimapIcon()
    ...
    if LDB and LibDBIcon then
        -- Método com LibDataBroker (Retail)
        ...
        LibDBIcon:Register("GoldWatch", minimapButton, GW.Settings)
        GW.UI.minimapIcon = LibDBIcon
        
        if not GW.Settings.showMinimap then
            LibDBIcon:Hide("GoldWatch")
        end
    else
        -- Método manual (Classic)
        local button = CreateFrame("Button", "GWMinimapButton", Minimap)
        button:SetSize(33, 33) -- Tamanho do rastreio do mouse
        button:SetFrameStrata("MEDIUM")
        button:SetPoint("CENTER", -72, -34) -- Posição que inicia o icone
        button:SetMovable(true)
        
        local icon = button:CreateTexture(nil, "BACKGROUND")
        icon:SetSize(20, 20) -- Icone moeda
        icon:SetTexture("Interface\\Icons\\inv_misc_coin_01")
        icon:SetPoint("CENTER", 0, 2)
        
        local overlay = button:CreateTexture(nil, "OVERLAY")
        overlay:SetSize(54, 54) -- Circulo da moeda
        overlay:SetTexture("Interface\\Minimap\\MiniMap-TrackingBorder")
        overlay:SetPoint("CENTER", 10, -10) -- Posicao circulo
        ...
        button:SetScript("OnEnter", function()
            ...
            GameTooltip:AddLine("GoldWatch")
            GameTooltip:AddLine(GW.L.GetString("MINIMAP_TOOLTIP"), 1, 1, 1, true)
            GameTooltip:Show()
        end)
...
function GW.UI.CreateConfigWindow()
    ...
    local locale = GW.Settings.locale or "enUS"
    if locale == "ptBR" then
    ...
    elseif locale == "ruRU" then
        GameTooltip:AddLine("1с = Плавнее (больше CPU)\n5с = Легче (меньше обновлений)", 1, 1, 1, true)
    else -- enUS
    ...
    -- Atualizar janela con valores iniciais
...
function GW.UI.UpdateConfigWindow()
	...
	info.text = GW.L.GetString("LANGUAGE_GERMAN")
        ...
        -- Adicionar Russo
        info.text = GW.L.GetString("LANGUAGE_RUSSIAN")
        info.value = "ruRU"
        info.func = function() 
            GW.Settings.locale = "ruRU"
            GW.UI.UpdateAllTexts()
            GW.History.UpdateHistoryTexts()
            print(format(GW.L.GetString("LOCALE_CHANGED"), GW.L.GetString("LANGUAGE_RUSSIAN")))
            GW.UI.UpdateConfigWindow()
        end
        UIDropDownMenu_AddButton(info)
    end)
...
function GW.UI.UpdateDisplay()
    ...
    -- Meta de Gold - CORREÇÃO: Mostrar valores precisos sem arredondamento incorreto
    if GW.DB and GW.DB.goal and GW.DB.goal.isActive then
        local progress = GW.DB.goal.progress or 0
        local target = GW.DB.goal.target or 0
        
        -- Converter para K (1K = 1000 gold = 10.000.000 cobre)
        local progressK = progress / 10000000
        local targetK = target / 10000000
        
        -- Mostrar sempre com uma casa decimal
        local progressDisplay = GW.Data.FormatKValue(progress)
        local targetDisplay = string.format("%.1fK", targetK)
        
        GW.UI.goalText:SetText(string.format("%s / %s", progressDisplay, targetDisplay))
    else
        GW.UI.goalText:SetText("0 / 0")
    end
...
function GW.UI.UpdateAllTexts()
    ...
    if GW.UI.labels then
        ...
        if GW.UI.labels.goal then
            GW.UI.labels.goal:SetText(GW.L.GetString("GOAL_LABEL"))
        end
    end
    ...
    GW.UI.goalBtn:SetText(GW.L.GetString("GOAL_BUTTON"))
...
function GW.UI.ShowHyperspawnAlert(percentage)
    ...
    -- Tocar som de alerta - CORREÇÃO: Adicionar canal "SFX"
    if GW.Settings.alertSound then
        PlaySound(GW.Settings.alertSound, "SFX")  -- Correção aplicada
    end
...
-- Sistema de Meta de Gold
function GW.UI.CreateGoalWindow()
    if GW.UI.goalFrame then return end
    
    local frame = CreateFrame("Frame", "GoldWatchGoalFrame", UIParent)
    ...
    frame.title = frame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    frame.title:SetPoint("TOP", 0, -15)
    frame.title:SetText(GW.L.GetString("GOAL_TITLE"))
    frame.title:SetTextColor(1, 0.82, 0)
    
    -- Label de entrada
    frame.inputLabel = frame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    frame.inputLabel:SetPoint("TOPLEFT", 20, -50)
    frame.inputLabel:SetText(GW.L.GetString("GOAL_INPUT_LABEL"))
    
    -- Campo de entrada
    frame.editBox = CreateFrame("EditBox", "GWGoalEditBox", frame, "InputBoxTemplate")
    frame.editBox:SetSize(100, 30)
    frame.editBox:SetPoint("LEFT", frame.inputLabel, "RIGHT", 10, 0)
    frame.editBox:SetNumeric(true)
    frame.editBox:SetAutoFocus(false)
    
    -- Botão Definir
    frame.setButton = CreateFrame("Button", nil, frame, "UIPanelButtonTemplate")
    frame.setButton:SetSize(100, 25)
    frame.setButton:SetPoint("TOP", frame.editBox, "BOTTOM", -150, -20)
    frame.setButton:SetText(GW.L.GetString("GOAL_SET_BUTTON"))
    frame.setButton:SetScript("OnClick", function()
        local value = frame.editBox:GetNumber()
        if value > 0 then
            GW.Data.SetGoal(value)
            frame:Hide()
        end
    end)
    ...
    frame.resetButton:SetPoint("TOP", frame.setButton, "BOTTOM", 170, 25)
    frame.resetButton:SetText(GW.L.GetString("GOAL_RESET_BUTTON"))
    frame.resetButton:SetScript("OnClick", function()
        StaticPopup_Show("GOLDWATCH_CONFIRM_RESET_GOAL")
        frame:Hide()
    end)
    
    -- Informações atuais
    frame.currentLabel = frame:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    frame.currentLabel:SetPoint("TOP", frame.resetButton, "BOTTOM", -84, -30)
    frame.currentLabel:SetText(GW.L.GetString("GOAL_CURRENT"))
    
    frame.remainingLabel = frame:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    frame.remainingLabel:SetPoint("TOP", frame.currentLabel, "BOTTOM", 0, -5)
    frame.remainingLabel:SetText(GW.L.GetString("GOAL_REMAINING"))
    
    GW.UI.goalFrame = frame
end

function GW.UI.ToggleGoalWindow()
    GW.UI.CreateGoalWindow()
    if GW.UI.goalFrame:IsShown() then
        GW.UI.goalFrame:Hide()
    else
        GW.UI.goalFrame:Show()
        GW.UI.UpdateGoalWindow()
        
        -- Adicionar atualização automática enquanto a janela estiver aberta
        GW.UI.goalFrame:SetScript("OnUpdate", function(self, elapsed)
            self.updateTimer = (self.updateTimer or 0) + elapsed
            if self.updateTimer > 1 then  -- Atualizar a cada 1 segundo
                GW.UI.UpdateGoalWindow()
                self.updateTimer = 0
            end
        end)
...
function GW.UI.UpdateGoalWindow()
    if not GW.UI.goalFrame then return end
    
    local progress, remaining, completed = GW.Data.GetGoalProgress()
    
    if GW.DB.goal.isActive then
        -- Converter para K (1K = 1000 gold = 10.000.000 cobre)
        local progressK = progress / 10000000
        local remainingK = remaining / 10000000
        
        -- Mostrar sempre com uma casa decimal, sem arredondamento
        local progressDisplay = GW.Data.FormatKValue(progress)
        local remainingDisplay = string.format("%.1fK", remainingK)
        
        GW.UI.goalFrame.currentLabel:SetText(
            string.format("%s %s", 
            GW.L.GetString("GOAL_CURRENT"), 
            progressDisplay)
        )
        
        GW.UI.goalFrame.remainingLabel:SetText(
            string.format("%s %s", 
            GW.L.GetString("GOAL_REMAINING"), 
            remainingDisplay)
        )
        
        if completed then
            GW.UI.goalFrame.remainingLabel:SetTextColor(0, 1, 0)
        else
            GW.UI.goalFrame.remainingLabel:SetTextColor(1, 1, 1)
        end
    else
        GW.UI.goalFrame.currentLabel:SetText(GW.L.GetString("GOAL_CURRENT") .. " 0")
        GW.UI.goalFrame.remainingLabel:SetText(GW.L.GetString("GOAL_REMAINING") .. " 0")
    end
end